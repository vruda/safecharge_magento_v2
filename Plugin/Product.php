...
class Product
{
    ...
    public function afterGetPrice(\Magento\Catalog\Model\Product $product, $result)
    {
        try {
            ...
            $nuvei_subscr_init_amount =
                $product->getCustomAttribute(\Nuvei\Payments\Model\Config::PAYMENT_SUBS_INTIT_AMOUNT);
        
            if (!empty($enabled_nuvei_subscr)
...
//            }
//
////            $enabled_nuvei_subscr =
//                  $product->getCustomAttribute(\Nuvei\Payments\Model\Config::PAYMENT_SUBS_ENABLE);
////            $nuvei_subscr_init_amount =
//                  $product->getCustomAttribute(\Nuvei\Payments\Model\Config::PAYMENT_SUBS_INTIT_AMOUNT);
////
////            if(
